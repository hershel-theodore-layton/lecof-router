...
<<TestChain\Discover>>
function slashed_literals_filter_test(
  ...
  return $chain->group(__FUNCTION__)
    ...
    ->test(
      'test_forwards_the_request_to_the_matching_slashed_literal',
      ()[defaults] ==> {
        $expect = $assert->rand();
        $assert->assertReturns(
          Lecof\slashed_literals(dict[
          ...
          $assert->request('/a/b/e/g'),
          $expect,
        );
      },
    )
    ->test('test_bails_on_a_partial_match', () ==> {
      $assert->assertBails(
        Lecof\slashed_literals(dict[
          'a/b/c/g' => Lecof\done(1),
          'a/b/d/g' => Lecof\done(2),
